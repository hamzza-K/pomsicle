...
from recipe.loader import insert_components_with_guid
...
def handle_recipe_create_custom(args, token=None):
    ...
    args.recipe_name   = "Assisted"
    args.unit_procedure = None 
    args.operation = None

    logger.info(f"Creating CUSTOM RECIPE with phases: {args.add}")
    insert_components_with_guid(args.add, out_path=args.template_name)
    print(f"Custom recipe created with: {'-> '.join(args.add)}")
...
def create_cli():
    ...
    # ----- pomsicle recipe create custom --add obj1 obj2 -----
